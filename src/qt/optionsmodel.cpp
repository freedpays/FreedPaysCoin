...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFreedPaysAmount"))
        settings.setValue("nAnonymizeFreedPaysAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeFreedPaysAmount = settings.value("nAnonymizeFreedPaysAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeFreedPaysAmount"))
        SoftSetArg("-anonymizeFreedPaysamount", settings.value("nAnonymizeFreedPaysAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeFreedPaysAmount:
            nAnonymizeFreedPaysAmount = value.toInt();
            settings.setValue("nAnonymizeFreedPaysAmount", nAnonymizeFreedPaysAmount);
            emit anonymizeFreedPaysAmountChanged(nAnonymizeFreedPaysAmount);
            break;
        default:
            break;
        }
