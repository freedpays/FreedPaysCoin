...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("FreedPays"))
        return false;
...
{
    // Convert FreedPays:// to FreedPays:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("FreedPays://"))
    {
        uri.replace(0, 12, "FreedPays:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FreedPays.lnk";
}
...
{
    return GetAutostartDir() / "FreedPays.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FreedPays-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FreedPays-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("FreedPays-Qt"));
