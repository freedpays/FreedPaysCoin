...
void Shutdown()
{
    ...
    RenameThread("freedpays-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: FreedPays.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: freedpaysd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeFreedPaysamount=<n> " + _("Keep N FreedPays anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FreedPays is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FreedPays is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FreedPays version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "FreedPays server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FreedPays") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart FreedPays to complete") << "\n";
    ...
    nAnonymizeFreedPaysAmount = GetArg("-anonymizeFreedPaysamount", 0);
    if(nAnonymizeFreedPaysAmount > 999999) nAnonymizeFreedPaysAmount = 999999;
    if(nAnonymizeFreedPaysAmount < 2) nAnonymizeFreedPaysAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeFreedPaysAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize FreedPays Amount %d\n", nAnonymizeFreedPaysAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1FreedPays+1000 == (.1FreedPays+100)*10
       10FreedPays+10000 == (1FreedPays+1000)*10
    */
