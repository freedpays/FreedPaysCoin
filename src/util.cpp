...
int nAnonymizeFreedPaysAmount = 500;
...
#else
    const char* pszModule = "FreedPays";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FreedPays
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FreedPays
    // Mac: ~/Library/Application Support/FreedPays
    // Unix: ~/.FreedPays
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FreedPays";
#else
    fs::path pathRet;
...
void createConf()       //Automatic FreedPays.conf generation
{
    srand(time(NULL));
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "FreedPays.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "freedpaysd.pid"));
